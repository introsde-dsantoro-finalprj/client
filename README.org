...
** DB Service
   ...
   EclipseLink has been used in order to map classes with the ER model via the JPA interface. Remote DB can be configured using EN_VARS, see: [[https://github.com/introsde-dsantoro-finalprj/dbws/blob/master/serviceSetup-example.sh][serviceSetup-example.sh]] script.
** Adapter Service
   - Name: adpws
   - Default Port: 6904
